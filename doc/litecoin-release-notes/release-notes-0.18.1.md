...
`/Applications/Catcoin-Qt` (on Mac) or `litecoind`/`litecoin-qt` (on
Linux).
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litecoin.org) the Catcoin Core
...
- A new `litecoin-wallet` tool is now distributed alongside Catcoin
